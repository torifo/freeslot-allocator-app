...
5. **初回ペアリングだけは競合を記録しない（レビュー反映）。** 検出には合意点（`lastSyncAt`）が要るので、一度も同期していない端末との初回マージでは検出が丸ごと止まり、衝突はすべて LWW で決着する。ここは仕様どおりで変更しない（初回に「全部競合」を出しても選びようがない）。代わりに、**初回ペアリング時にハブ側で `data.json.first-pair-<ts>` の控えを残す**ことを推奨し、Plan 3b で実装した（`SyncEngine.sync` が `incoming.lastSyncAt == null` かつハブ文書が空でないときに 1 度だけコピーする）。`.bak` は毎回上書きされるので、初回だけの巻き戻し先としては別名の控えが要る。ユーザーには「初回ペアリングの前後は、必要ならこの控えから戻せる」と伝えれば足りる。
6. **Web の擬似端末を purge のカットオフ計算に入れるか。** 推奨: **入れない**（A-6）。永続ストアを持たないので取り残される概念が無く、入れると「ブラウザを開かない期間」で purge が止まる。表示用の `webClients` に留め、`forget_device` で消せるようにする。
