...
- 競合の記録と解決: 同じ項目を PC とスマホ（またはブラウザ）の両方で直していた場合、同期は今までどおり完了したうえで、採用されなかった方を「競合」として記録します。「設定 › PC と同期 › 競合」から一覧を開き、どちらの版を採用するか（あるいは現状のまま）をあとから選べます。採用した版は新しい編集として書き戻るので、次の同期で他の端末にも伝わります。競合レコードを知らない古いアプリはこのフィールドを落としますが、ハブ側は常に両者の和集合を保つため PC の記録は消えません。
