...
register('resolve_conflict', "Resolve one conflict by adopting the PC version (adopt: 'hub' — the MCP hub or the browser it serves), the phone version (adopt: 'device'), or leaving the current state as-is. Adopting writes that snapshot back as a fresh edit (a new clock), so it propagates to the phone and the browser on the next sync; adopting a tombstone deletes the entity again", schemas.resolveConflict.shape, (i) => wrap(() => tools.resolveConflict(i)));
register('resolve_all_conflicts', 'Destructive: resolve every open conflict the same way in one write (all or nothing). With entityType, records of other types are left untouched and counted in neither resolved nor skipped. dryRun reports what would change without writing', schemas.resolveAllConflicts.shape, (i) => wrap(() => tools.resolveAllConflicts(i)));
